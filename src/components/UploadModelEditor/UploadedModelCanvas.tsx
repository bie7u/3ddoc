...
export interface MeshPickResult {
  /** Name of the clicked mesh (empty string if unnamed) */
  meshName: string;
  /** World-space centre of the clicked mesh's bounding box */
  focusPoint: [number, number, number];
  /** Camera state at the moment of the click */
...
// ─── Model renderer – called only when a valid blobUrl is ready ───────────────
  ...
  const { clonedScene, origEmissive } = useMemo(() => {
    ...
    clone.traverse((child) => {
      if (child instanceof THREE.Mesh) {
        // Clone materials so we own them independently
  ...
  const handleClick = (e: ThreeEvent<MouseEvent>) => {
    ...
    const camPos: CameraPosition = snap
      ? {
          ...
          targetX: snap.target.x, targetY: snap.target.y, targetZ: snap.target.z,
        }
  ...
  useEffect(() => {
    if (cameraMode === 'auto' && targetCamera) {
      ...
      targetCamLookAt.current.set(
        targetCamera.targetX ?? 0,
        targetCamera.targetY ?? 0,
        targetCamera.targetZ ?? 0,
      );
    }
  }, [targetCamera, cameraMode]);

  useFrame(() => {
    // Publish live camera state for "capture camera" button
    if (cameraStateRef) {
...
// ─── Blob URL conversion hook (used outside Canvas) ──────────────────────────
